...
const Budget: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBudgets = () => {
      fetch("http://localhost:1337/api/budgets")
        .then((res) => {
          if (!res.ok) {
  ...
  const handleDeleteBudget = async (documentId: string) => {
    try {
      alert("Are you sure you want to delete this budget?")
      await axios.delete(`http://localhost:1337/api/budgets/${documentId}`);
      setBudgets(budgets.filter((budget) => budget.documentId !== documentId));
    } catch (error) {
      console.error(error);
    }
  };
                    ...
                    <article className="py-3 px-5 border-l-8 border-teal-500">
                      ...
                      <section className="flex items-center flex-wrap mt-5">
                        ...
                        <span className="inline-flex items-center leading-none text-sm bg-gray-300 rounded-full p-2 cursor-pointer" title="Delete">
                          <FaTrash className="w-4 h-4" onClick={() => handleDeleteBudget(budget.documentId)} />
                        </span>
                      </section>
                    </article>
                  </article>
