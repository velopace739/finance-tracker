...
const Income: React.FC<IncomeProps> = ({ refreshCashflow }) => {
  ...
  const fetchIncomes = () => {
    fetch("http://localhost:1337/api/incomes")
      .then((res) => {
        if (!res.ok) {
  ...
  const handleDeleteIncome = async (documentId: string) => {
    try {
      await axios.delete(`http://localhost:1337/api/incomes/${documentId}`);
      setIncomes(incomes.filter((income) => income.documentId !== documentId));
      refreshCashflow();
    } catch (error) {
      console.error(error);
    }
                    ...
                    {dropdownOpen === income.id && (
                      <div className="origin-top-right absolute right-0 mt-0 mr-2 w-32 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                        <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                          ...
                          <div
                            className="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer"
                            onClick={() => handleDeleteIncome(income.documentId)}>
