...
interface IncomeFormProps {
  isOpen: boolean;
  onClose: () => void;
  income: { id: number, documentId: string, description: string, amount: number } | null;
...
const IncomeForm: React.FC<IncomeFormProps> = ({ isOpen, onClose, income, refreshCashflow }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (income) {
        await axios.put(`http://localhost:1337/api/incomes/${income.documentId}`, { data: incomeData });
      } else {
